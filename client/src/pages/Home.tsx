...
import { getAnalyticsConsent, loadAnalytics, setAnalyticsConsent, type AnalyticsConsent } from "@/lib/analytics";
...
const featuredImages: ImageRecord[] = [
  { src: "/manus-storage/page-02_1604826d.webp", title: "ВОДА КАМЕНЬ ТОЧИТ", meta: "01 / УЛАН-УДЭ · 2019", tag: "Фестиваль «Бурятия в красках»", size: "wide" },
  { src: "/manus-storage/page-05_8f0a7114.webp", title: "Оформление скейт-парка", meta: "02 / НОЯБРЬСК · 2020", tag: "Проект по инициативе местных НКО", size: "tall" },
  { src: "/manus-storage/page-07_4164bd5e.webp", title: "Коктебель", meta: "03 / КОКТЕБЕЛЬ", tag: "В рамках фестиваля «Щит Кафы»", size: "standard" },
  { src: "/manus-storage/page-08_b1faf415.webp", title: "THE SEED", meta: "04 / САНКТ-ПЕТЕРБУРГ · 2020", tag: "Фестиваль «Уличная Россия»", size: "standard" },
  { src: "/manus-storage/page-10_b7dc6ffb.webp", title: "ШОРСКИЙ ШАМАН", meta: "05 / ШЕРЕГЕШ · 2021", tag: "Фестиваль «Окрашено»", size: "wide" },
  { src: "/manus-storage/page-14_d3cc171c.webp", title: "ЦАРЬ ПУТЕЙ", meta: "06 / ЗИМА · 2021", tag: "Фестиваль Energy Nation", size: "standard" },
  { src: "/manus-storage/page-17_9cc55f2f.webp", title: "683002", meta: "07 / ПЕТРОПАВЛОВСК-КАМЧАТСКИЙ · 2022", tag: "Камчатка", size: "tall" },
  { src: "/manus-storage/page-21_c54060e5.webp", title: "В ГОРЫ КАК ДОМОЙ / СНОУБОРД — ФИШКА КАМЧАТКИ / МОМЕНТ / АКВАТОРИЯ", meta: "08 / ПЕТРОПАВЛОВСК-КАМЧАТСКИЙ · 2023", tag: "Серия фасадов", size: "wide" },
  { src: "/manus-storage/page-25_479c2f9d.webp", title: "У ЛИС", meta: "09 / ВЛАДИВОСТОК · МЫС ЧУМАКА · 2023", tag: "Открытая стена", size: "wide" },
  { src: "/manus-storage/page-30_8eaf99a8.webp", title: "DAKING jam", meta: "10 / DAKING JAM · 2023", tag: "DAKING jam", size: "standard" },
  { src: "/manus-storage/page-31_7885a7f0.webp", title: "СО СВОИМ САМОВАРОМ / 2", meta: "11 / SANTIAGO · 2023", tag: "Santiago", size: "wide" },
];

const archiveImages: ImageRecord[] = [
  ...featuredImages,
  ...[1, 3, 4, 6, 9, 11, 12, 13, 15, 16, 18, 19, 20, 22, 23, 24, 26, 27, 28, 29, 32].map((page) => ({
    src: `/manus-storage/page-${String(page).padStart(2, "0")}_${({ 1: "d1846c42", 3: "bacd3d99", 4: "e392702e", 6: "bf4d4243", 9: "3513f603", 11: "b5cd0c17", 12: "2f383203", 13: "8fee6b3e", 15: "81b33e37", 16: "cf8aefd0", 18: "dcc9bd0b", 19: "feb486c8", 20: "92e7aee6", 22: "7f808007", 23: "9817fab5", 24: "152faa32", 26: "d51bef29", 27: "2ff25cdd", 28: "04265831", 29: "fc71143c", 32: "9c47dea4" } as Record<number, string>)[page]}.webp`,
...
].map(([file, label, size], index) => ({ src: `/manus-storage/${file}`, title: `СТЕНА / ${label}`, meta: `СТЕНЫ / ${String(index + 1).padStart(2, "0")} / 34`, tag: "Пользовательский архив EffectGraff", size: size as ImageRecord["size"] }));
  ...
  return (
    <div className="site-shell">
      <header className="site-header">
        <button className="brand-lockup" onClick={() => scrollTo("top")} aria-label="EffectGraff — наверх"><span className="brand-mark"><img src="/manus-storage/eg-club-logo-1_42f150e4.png" alt="" /></span><span className="brand-word">EFFECT<span>GRAFF</span></span></button>
      ...
      <audio ref={audioRef} loop preload="none" src="/manus-storage/effectgraff-calm-ethno-loop_ebebb5c1.wav" />
      ...
      <main id="top">
        <section className="hero-section">
          ...
          <div className="hero-visual"><div className="hero-wash" /><div className="hero-image-wrap"><img src="/manus-storage/page-02_1604826d.webp" alt="Мурал EffectGraff на городской стене" /><div className="hero-scan-layer" aria-hidden="true" /><div className="registration-mark" aria-hidden="true"><span /><span /></div><div className="hero-stamp"><b>EG—01</b><br /><span>STREET<br />ARCHIVE</span></div><div className="eg-seal" aria-hidden="true"><img src="/manus-storage/eg-club-logo-1_42f150e4.png" alt="" /><small>FIELD<br />NOTE</small></div></div><div className="hero-side-note">55°45′N<br />37°37′E</div></div>
